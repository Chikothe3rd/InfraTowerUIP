...
export default function SensorOperations() {
  ...
  const motionDetected = latestReading?.motionDetected ?? false
  const batterySoH = latestReading?.batterySoH ?? 100.0

  // Diagnostics calculations
  const healthScore = Math.round((batterySoH + (enclosureTemp > 45 ? 78 : (enclosureTemp > 38 ? 90 : 98))) / 2)
  const wearFactor = enclosureTemp > 45 ? 42 : (batterySoH < 85 ? 35 : 12)
  const expectedLifespan = batterySoH < 85 ? '1.8 Years' : (enclosureTemp > 45 ? '3.2 Years' : '7.5 Years')
          ...
          <div style={styles.rightCol}>
            ...
            <div className="card">
              ...
              <div style={styles.sensorStatesList}>
                ...
                <div style={styles.sensorStateItem}>
                  <ShieldCheck size={18} color={motionDetected ? 'var(--accent-danger)' : 'var(--accent-success)'} />
                  <div style={styles.sensorItemText}>
                    <span>Perimeter Motion Sensor</span>
                    <b style={{ color: motionDetected ? 'var(--accent-danger)' : 'inherit' }}>
                      {motionDetected ? '🔴 MOTION DETECTED IN SECTOR' : '🟢 RACK PERIMETER SECURE'}
                    </b>
                  </div>
                </div>
              </div>
            ...
            <div className="card">
              ...
              <div style={styles.diagnosticGrid}>
                <div style={styles.diagItem}>
                  <span style={styles.diagLabel}>Battery Bank SoH</span>
                  <span style={{ ...styles.diagValue, color: batterySoH < 85 ? 'var(--accent-danger)' : 'var(--accent-success)' }}>{batterySoH.toFixed(1)}%</span>
                </div>
                <div style={styles.diagItem}>
                  <span style={styles.diagLabel}>Expected Lifespan</span>
              ...
              <div style={styles.recommendationAlert}>
                ...
                <div style={styles.recText}>
                  ...
                  <span>
                    {batterySoH < 85
                      ? 'Scheduler recommendation: Battery Bank State of Health has dropped below 85.0% threshold. Dispatch regional technician immediately for replacements.'
                      : (enclosureTemp > 45 
