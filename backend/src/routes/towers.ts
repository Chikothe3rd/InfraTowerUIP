...
router.get('/:id/telemetry', async (req: AuthRequest, res: Response) => {
  const { id } = req.params
...
router.get('/:id/power-timeline', async (req: AuthRequest, res: Response) => {
  const { id } = req.params
...
router.patch('/:id/config', authorizeRoles('ADMIN'), async (req: AuthRequest, res: Response) => {
  const { id } = req.params
