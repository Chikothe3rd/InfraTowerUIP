...
function getFilterParams(req: AuthRequest) {
...
router.get('/uptime', async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const dailyData: Record<string, { total: number; online: number }> = {}
...
router.get('/power', async (req: AuthRequest, res: Response) => {
  const { whereClause } = getFilterParams(req)
...
router.get('/fuel', async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const dailyData: Record<string, { total: number; genActiveCount: number; maxFuel: number; minFuel: number }> = {}
    ...
    const result = Object.entries(dailyData).map(([date, stats]) => {
      ...
      const theftCount = theftAlerts.filter((a: any) =>
...
router.get('/power-history', async (req: AuthRequest, res: Response) => {
  const { whereClause } = getFilterParams(req)
  ...
  try {
    ...
    const grouped: Record<string, { grid: number; solar: number; generator: number; total: number }> = {}
...
router.get('/power-metrics', async (req: AuthRequest, res: Response) => {
  const { whereClause } = getFilterParams(req)
