...
router.post('/query', async (req, res) => {
    ...
    try {
        ...
        // 2. Average fuel in region
            ...
            let regionKey = '';
            if (normalizedQuery.includes('east'))
                regionKey = 'East';
            else if (normalizedQuery.includes('west'))
                regionKey = 'West';
            else if (normalizedQuery.includes('south'))
                regionKey = 'South';
            else if (normalizedQuery.includes('central'))
                regionKey = 'Central';
            else if (normalizedQuery.includes('north-west'))
                regionKey = 'North-West';
            else if (normalizedQuery.includes('north-east'))
                regionKey = 'North-East';
                ...
                if (total > 0) {
                    ...
                    if (sla < 99.5) {
                        breachingTowers.push({
                            ...
                            penaltyExposure: `$${(t.tenants.reduce((sum, x) => sum + x.monthlyRevenue, 0) * 0.1).toFixed(2)}`
                        });
                    }
                }
            }
