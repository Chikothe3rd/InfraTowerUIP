...
async function getTowerCommercialData() {
    ...
    return towers.map((tower) => {
        ...
        if (telemetryCount > 0) {
            ...
            uptimeCount = tower.telemetry.filter((t) => t.gridPower > 0 || t.solarPower > 0 || t.generatorRunning).length;
        }
        const slaPercentage = telemetryCount > 0
            ? parseFloat(((uptimeCount / telemetryCount) * 100).toFixed(2))
            : 100.00;
        // 2. Revenue calculation
        const monthlyRevenue = tower.tenants.reduce((sum, t) => sum + t.monthlyRevenue, 0);
        ...
        return {
            ...
            tenants: tower.tenants.map((t) => t.clientName),
            tenancyRatio,
            monthlyRevenue,
...
router.get('/summary', async (req, res) => {
    try {
        ...
        const totalRevenue = commercialData.reduce((sum, t) => sum + t.monthlyRevenue, 0);
        const totalCost = commercialData.reduce((sum, t) => sum + t.operationalCost, 0);
        const totalProfit = totalRevenue - totalCost;
        const totalTenants = commercialData.reduce((sum, t) => sum + t.tenantsCount, 0);
        const avgTenancyRatio = parseFloat((commercialData.reduce((sum, t) => sum + t.tenancyRatio, 0) / commercialData.length).toFixed(2));
        const breachingTowers = commercialData.filter((t) => t.isSlaBreached);
        const totalSlaPenaltyExposure = breachingTowers.reduce((sum, t) => sum + t.penaltyExposure, 0);
        const underutilizedSites = commercialData.filter((t) => t.tenantsCount < 2).length;
        return res.json({
...
router.get('/towers', async (req, res) => {
    try {
        ...
        commercialData.sort((a, b) => b.monthlyRevenue - a.monthlyRevenue);
